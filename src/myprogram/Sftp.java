...
public class Sftp {
	 ...
	 * count=0; long max=0; public void init(String info, long max){ this.max=max;
	 * if(frame==null){ frame=new JFrame(); frame.setSize(200, 20); progressBar =
	 * new JProgressBar(); } count=0; frame.setTitle(info);
	 * progressBar.setMaximum((int)max); progressBar.setMinimum((int)0);
	 * progressBar.setValue((int)count); progressBar.setStringPainted(true); JPanel
	 * p=new JPanel(); p.add(progressBar); frame.getContentPane().add(progressBar);
